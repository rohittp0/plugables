...
class HtmlReportRendererTest {
    ...
    fun `embeds preview JSON with overlays`() {
        val rendered = listOf(RenderedPreview(previewSpec, "previews/Codeview_HomePreview_0.png", nodes, listOf("Welcome back")))
        ...
        assertTrue(html.contains("\"renderedTexts\":[\"Welcome back\"]"))
    }

    @Test
    fun `previews without image emit null imagePath`() {
        val rendered = listOf(RenderedPreview(previewSpec, null, emptyList(), emptyList()))
    ...
    fun `script-tag breakouts in displayName are escaped`() {
        ...
        val rendered = listOf(RenderedPreview(malicious, null, emptyList(), emptyList()))
        val html = HtmlReportRenderer.render(rendered, imageDimensions = emptyMap(), ideScheme = "idea")
